...
GLuint  LibertyStatueDL;				// declare as a global
GLuint  CatDL;
GLuint  TeapotDL;
unsigned char * Texture;	// the texels
unsigned int    BronzeTex;	// the texture object
unsigned int    PumpkinTex;
...
Display( )
{
	...
	gluLookAt( 4.f, 3.f, 4.f,     0.f, 0.f, 0.f,     0.f, 1.f, 0.f );
	...
	// Set static light options
	SetPointLight(GL_LIGHT1, 0,-3,0,  1,1,1);
	SetSpotLight(GL_LIGHT2, -1,3,3,  1,-3,-3,  1,1,0);
	...
	glPushMatrix();
		...
		if (Light0On){
			//Draw Light 0
			glPushMatrix();
				// Translate from statue center to torch
				glTranslatef(-.35, 2.65, .28);
				//Set light options
				SetPointLight(GL_LIGHT0, 0, 0, 0, 1,.86,.66);
				glColor3f(1,.86,.66);
				glDisable(GL_LIGHTING);
				glutSolidSphere(.1,10,10);
				glEnable(GL_LIGHTING);
		...
		glBindTexture(GL_TEXTURE_2D, BronzeTex);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		glCallList(LibertyStatueDL);	
...
InitGraphics( )
{
	...
	int width, height;
	Texture = BmpToTexture( (char *)"Liberty-GreenBronze-1.bmp", &width, &height );
	if( Texture == NULL )
			fprintf( stderr, "Cannot open texture '%s'\n", "Liberty-GreenBronze-1.bmp" );
	else
			fprintf( stderr, "Width = %d ; Height = %d\n", width, height );

	glPixelStorei( GL_UNPACK_ALIGNMENT, 1 );
	glGenTextures( 1, &BronzeTex );
	glBindTexture( GL_TEXTURE_2D, BronzeTex );
	...
	// Texture = BmpToTexture( (char *)"Pumpkin.bmp", &width, &height );
	// if( Texture == NULL )
	// 		fprintf( stderr, "Cannot open texture '%s'\n", "Pumpkin.bmp" );
	// else
	// 		fprintf( stderr, "Width = %d ; Height = %d\n", width, height );
	...
	// glPixelStorei( GL_UNPACK_ALIGNMENT, 1 );
	// glGenTextures( 1, &PumpkinTex );
	// glBindTexture( GL_TEXTURE_2D, PumpkinTex );
	// glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
	// glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
	// glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
	// glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
...
InitLists( )
{
	...
	LibertyStatueDL = glGenLists( 1 );
	glNewList( LibertyStatueDL, GL_COMPILE );
	LoadObjFile( (char *)"LibertyStatue.obj" );
	glEndList( );
...
SetSpotLight( int ilight, float x, float y, float z, float xdir, float ydir, float zdir, float r, float g, float b )
{
	...
	glLightf( ilight, GL_SPOT_CUTOFF, 25. );
	glLightfv( ilight, GL_AMBIENT, Array3( 0., 0., 0. ) );
	glLightfv( ilight, GL_DIFFUSE, Array3( r, g, b ) );
	glLightfv( ilight, GL_SPECULAR, Array3( r, g, b ) );
	glLightf ( ilight, GL_CONSTANT_ATTENUATION, 1. );
